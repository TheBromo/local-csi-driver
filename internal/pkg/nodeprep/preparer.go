...
// Package nodeprep prepares the Azure ephemeral resource disk of a node for
// use as an LVM volume group backing local-csi-driver volumes.
//
// It runs in a privileged init container before the CSI driver starts and
// converts the cloud-init owned /mnt resource-disk layout into a tagged LVM
// volume group. Every destructive action is gated behind explicit
// configuration and extensive preflight checks; unrecognized or ambiguous
// disk states fail closed before any write.
package nodeprep
...
// vgNamePattern restricts volume group names to LVM-safe characters.
...
func New(cfg Config, host Runner) (*Preparer, error) {
	...
	if !vgNamePattern.MatchString(cfg.VolumeGroupTag) {
		return nil, fmt.Errorf("invalid volume group tag %q", cfg.VolumeGroupTag)
	}
...
// diskState classifies the resource disk layout.
type diskState int
...
// checkHostCommands verifies all commands needed later are available before
...
// waitCloudInit waits for cloud-init to finish so preparation does not race
// the module that formats and mounts the resource disk.
